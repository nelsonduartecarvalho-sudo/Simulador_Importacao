...
def extrair_dados_mobile_de(url):
    """Extrai os dados consumindo os dados JSON internos do anúncio."""
    try:
        ...
        # Pedido HTTP simples via ScraperAPI (sem render=true para não dar erro 500)
        payload = {"api_key": SCRAPER_API_KEY, "url": target_url}

        res = requests.get(
            "http://api.scraperapi.com", params=payload, timeout=25
        )
        ...
        # Extração de Preço (€)
        preco = 16999.00
        m_preco = re.search(r'"price":\s*\{\s*"gross":\s*([\d\.]+)', html_content) or re.search(r"€\s?([\d\.]+)", html_content)
        if m_preco:
            preco = float(m_preco.group(1).replace(".", ""))

        # Extração de Cilindrada (cm³)
        cc = 1200
        m_cc = re.search(r'capacity["\']?:\s*["\']?(\d+)', html_content) or re.search(r"([\d\.]+)\s?cm³", html_content)
        if m_cc:
            cc = int(m_cc.group(1).replace(".", ""))

        # Extração de CO2 (g/km)
        co2 = 115
        m_co2 = re.search(r'emission["\']?:\s*["\']?(\d+)', html_content) or re.search(r"([\d]+)\s?g/km", html_content, re.IGNORECASE)
        if m_co2:
            co2 = int(m_co2.group(1))

        # Extração de Idade (Anos)
        idade = 4
        m_ano = re.search(r'firstRegistration["\']?:\s*["\']?(\d{4})', html_content) or re.search(r"(0[1-9]|1[0-2])\/((?:19|20)\d{2})", html_content)
        if m_ano:
            ano_val = m_ano.group(1) if len(m_ano.group(1)) == 4 else m_ano.group(2)
            idade = max(0, 2026 - int(ano_val))

        # Extração de Combustível
        combustivel = "Gasolina"
        if re.search(r"Diesel", html_content, re.IGNORECASE):
            combustivel = "Diesel"
        elif re.search(r"Hybrid", html_content, re.IGNORECASE):
            combustivel = "Híbrido"
        elif re.search(r"Electric", html_content, re.IGNORECASE):
...
if st.button("🔎 Extrair Dados do Anúncio"):
    if url_link:
        with st.spinner("A importar dados do anúncio..."):
